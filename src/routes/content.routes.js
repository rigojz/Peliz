...
/**
 * Proxy del stream para evitar bloqueos de CORS/Referer
 * GET /stream?url=https://....m3u8
 */
router.get(
  "/stream",
  asyncHandler(async (req, res) => {
    ...
    if (!streamUrl) {
      throw new ApiError(400, "Falta el parametro url");
    }
    ...
    try {
      ...
      throw new ApiError(502, "No se pudo obtener el stream");
    }
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Convertimos la URL real a nuestra URL de proxy
    const proxiedUrl = `/api/v1/content/stream?url=${encodeURIComponent(directUrl)}`;
    const isHls = /\.m3u8|\.txt/i.test(directUrl);
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .msg { color: #fff; text-align: center; margin-top: 40vh; font-family: system-ui; }
  </style>
...
<body>
  ...
  <script>
    ...
    const src = ${JSON.stringify(proxiedUrl)};
    const isHls = ${isHls};

    function showError(msg) {
      document.body.innerHTML = '<div class="msg">' + msg + '</div>';
    }
    ...
    if (isHls && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (e, data) => {
        if (data.fatal) showError('Error al cargar el video. Prueba otro servidor.');
      });
